...
// Centraliza o icone "i" de ajuda para que tenham o mesmo estilo.
function tooltipIcon(string $title): string
{
    return '<span class="help-tooltip" data-toggle="tooltip" data-html="true" title="' .
        htmlspecialchars($title, ENT_QUOTES, 'UTF-8') .
        '"><i class="fa-solid fa-circle-info tooltip-icon" aria-hidden="true"></i></span>';
}
...
// Sensibilidade - valores padrão
$variacoes = [-15, -10, -5, 0, 5, 10, 15];

$tabelaSensibilidade = [];
$tabelaSensibilidadeRoiMin = 0;
$tabelaSensibilidadeRoiMax = 0;
$roiBaseSensibilidade = 0;
$tabelaSensibilidadeCenariosPositivos = 0;
$tabelaSensibilidadeTotalCenarios = 0;

foreach ([
    'Lucro_Vista',
    'Desembolso_Total_Vista',
    'Taxa_Lucro_Vista',
    'Lance_Rentabilidade_Vista',
    'TIR_AM_Vista',
    'TIR_AA_Vista',
    'VPL_Vista',
    'Retorno_Aluguel_Vista',
    'Lucro_Price',
    'Desembolso_Total_Price',
    'Taxa_Lucro_Price',
    'Lance_Rentabilidade_Price',
    'TIR_AM_Price',
    'TIR_AA_Price',
    'VPL_Price',
    'Retorno_Aluguel_Price',
    'Lucro_SAC',
    'Desembolso_Total_SAC',
    'Taxa_Lucro_SAC',
    'Lance_Rentabilidade_SAC',
    'TIR_AM_SAC',
    'TIR_AA_SAC',
    'VPL_SAC',
    'Retorno_Aluguel_SAC',
    'Lucro_Parcelado',
    'Desembolso_Total_Parcelado',
    'Taxa_Lucro_Parcelado',
    'Lance_Rentabilidade_Parcelado',
    'TIR_AM_Parcelado',
    'TIR_AA_Parcelado',
    'VPL_Parcelado',
    'Retorno_Aluguel_Parcelado',
] as $resultadoCampo) {
    $$resultadoCampo = '';
}
...
if ($requestMethod === "POST") {
    ...
    $rentabilidade = isset($_POST["rentabilidade"]) ? $_POST["rentabilidade"] : 0;
...
if ($requestMethod === "POST") {
    ...
    // Base para análise de sensibilidade. Precisa vir depois dos cálculos das modalidades,
    // para que juros, prazo e imposto já estejam disponíveis.
    $arrematacaoBaseSensibilidade = $arrematacao;
    $vendaBaseSensibilidade = $venda;
    include __DIR__ . '/sensibilidade.php';

    $formatarLanceRentabilidade = function ($valor) {
        if ($valor === null) {
            return 'Nao viavel';
        }
    ...
    $Lance_Rentabilidade_Vista = $formatarLanceRentabilidade($tabelasSensibilidadePorModalidade['vista']['lanceRentabilidade'] ?? null);
    $Lance_Rentabilidade_Price = $formatarLanceRentabilidade($tabelasSensibilidadePorModalidade['price']['lanceRentabilidade'] ?? null);
    $Lance_Rentabilidade_SAC = $formatarLanceRentabilidade($tabelasSensibilidadePorModalidade['sac']['lanceRentabilidade'] ?? null);
    $Lance_Rentabilidade_Parcelado = $formatarLanceRentabilidade($tabelasSensibilidadePorModalidade['parcelado']['lanceRentabilidade'] ?? null);
...
<div id="main" style="padding-bottom: 90px;">
    ...
    <div class="container conteudo-com-espaco">
        ...
        <!-- Premissas -->
        <form method="post" action="#resultado-container" id="myForm">

            <div class="retangulo">
                <div class="container content-button">
                    <div class="button-group">
                        <button type="button" class="btn btn-outline-primary leftbutton"
                            id="salvarButton">Salvar</button>
                        <button type="button" class="btn btn-outline-primary leftbutton"
                            id="recuperarButton">Abrir</button>
                        <button type="button" class="btn btn-outline-primary leftbutton"
                            id="imprimirButton">Imprimir</button>
                    </div>
                    <div class="button-group">
                        <button type="button" class="btn btn-outline-primary rightbutton"
                            id="zerarButton">Limpar</button>
            ...
            <div id="premissas-section">
                ...
                <div class="form-group">
                    ...
                    <span data-toggle="tooltip" data-html="true"
                        title="Insira o valor que você espera pagar pelo imóvel no leilão.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control campo-conversao" id="valor1" name="valor1"
                        placeholder="Digite o valor em reais" value="<?= $arrematacao ?>" required>
                ...
                <div class="form-group">
                    ...
                    <span data-toggle="tooltip" data-html="true"
                        title="Informe o valor pelo qual você planeja vender o imóvel.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="valor2" name="valor2"
                        placeholder="Digite o valor em reais" value="<?= $venda ?>" required>
                    <span class="error-message" id="valor2-error"></span>
                </div>

                <div class="form-group">
                    <label for="rentabilidade">Rentabilidade Desejada (%)</label>
                    <!-- Target ROI used by future bid guidance; currently persisted/formatted with the form data. -->
                    <?= tooltipIcon('Informe a rentabilidade desejada sobre o investimento e indicaremos o valor de lance necessario para atingi-la.') ?>
                    <input type="text" class="form-control" id="rentabilidade" name="rentabilidade"
                        placeholder="Digite a rentabilidade desejada" value="<?= $rentabilidade ?>" required>
                    <span class="error-message" id="rentabilidade-error"></span>
                </div>

                <div class="form-group">
                    <label for="parcelas">Prazo de Vendas (meses)</label>
                    <input type="number" class="form-control" id="parcelas2" name="parcelas2"
                        placeholder="Digite o tempo do prazo em meses" value="<?= $prazo ?>" required>
                    <span class="error-message" id="parcelas2-error"></span>
                </div>

                <div class="form-group">
                    <label for="porcentagem1">Entrada (%)</label>
                    <input type="text" class="form-control" id="porcentagem1" name="porcentagem1"
                        placeholder="Digite a porcentagem" value="<?= $entrada ?>" required>
                    <span class="error-message" id="porcentagem1-error"></span>
                </div>
                <div class="form-group">
                    <label for="parcelas">Número de Parcelas</label>
                    <span data-toggle="tooltip" data-html="true"
                        title="Indique o número de parcelas do pagamento, se for à vista este número deve ser 1">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="number" class="form-control" id="parcelas1" name="parcelas1"
                        placeholder="Digite o número de parcelas" value="<?= $parcela ?>" required>
                    <span class="error-message" id="parcelas1-error"></span>
                </div>
                <div class="form-group">
                    <label for="porcentagem2">Taxa de Juros (% Anual)</label>
                    <span data-toggle="tooltip" data-html="true"
                        title="Adicione a taxa de juros do financiamento ou, em arrematações judiciais, a taxa de correção monetária aplicável.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="porcentagem2" name="porcentagem2"
                        placeholder="Digite a porcentagem" value="<?= $juros_anual ?>" required>
                    <span class="error-message" id="porcentagem2-error"></span>
                </div>
                <div class="form-group">
                    <label for="porcentagem4">Taxa de Mercado para Comparação (% Mensal)</label>
                    <span data-toggle="tooltip" data-html="true"
                        title="Insira uma taxa de referência como CDB ou Selic, para comparar o resultado do investimento.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="porcentagem4" name="porcentagem4"
                        placeholder="Digite a porcentagem" value="<?= $desconto ?>" required>
            ...
            <div id="custos-section">
                ...
                <div>
                    ...
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                        title="Detalhe os custos associados à compra do imóvel, preencha apenas o campo em porcentagem ou se preferir os valores absolutos.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                </div>
                ...
                <div class="row g-2">
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="comissaoEmReais" name="comissaoEmReais"
                                placeholder="R$1000,00" step="0.01" value="<?= $comissao ?>" required>
                    ...
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="comissaoEmPorcentagem" step="0.01"
                                name="comissaoEmPorcentagem" placeholder="5%" value="<?= $comissaoporc ?>">
                            <label for="comissaoEmPorcentagem">Comissão do Leiloeiro em %</label>
                            <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                                title="Essa informação normalmente é encontrada nos editais dos leilões, é comum ser 5%.">
                                <i class="fa-solid fa-circle-info tooltip-icon"></i>
                            </span>
                        </div>
                    </div>
                ...
                <div class="row g-2">
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="ITBIEmReais" name="ITBI" placeholder="R$1000,00"
                                step="0.01" value="<?= $ITBI ?>" required>
                    ...
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="ITBIEmPorcentagem" step="0.01" name="ITBIporc"
                                placeholder="5%" value="<?= $ITBIporc ?>">
                            <label for="ITBIEmPorcentagem">ITBI em %</label>
                            <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                                title="Insira a alíquota do Imposto sobre Transmissão de Bens Imóveis, esta costuma ser variar de 1% a 5% conforme o município.">
                                <i class="fa-solid fa-circle-info tooltip-icon"></i>
                            </span>
                        </div>
                    </div>
                ...
                <div class="row g-2">
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" class="form-control campo-conversao" id="asses1EmReais" step="0.01"
                                name="asses1" placeholder="R$1000,00" value="<?= $asses1 ?>" required>
                    ...
                    <div class="col-md">
                        <div class="form-floating">
                            <input type="text" step="0.01" class="form-control campo-conversao" id="asses1EmPorcentagem"
                                name="asses1porc" placeholder="5%" value="<?= $asses1porc ?>" max="100">
                            <label for="asses1EmPorcentagem">Assessoria em %</label>
                            <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                                title="Se utilizar serviços de assessoria, informe o valor a ser desembolsado no momento da aquisição do imóvel.">
                                <i class="fa-solid fa-circle-info tooltip-icon"></i>
                            </span>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                        title="São as dívidas que podem acompanhar o imóvel após a arrematação, normalmente essa informação está no edital. *Esse valor não é descontado no Ganho de Capital.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="dividaprop" name="dividaprop"
                        placeholder="Digite o valor da sua divida" value="<?= $dividaprop ?>" required>
                    <span class="error-message" id="dividaprop-error"></span>
                </div>
                <div class="form-group">
                    <label for="registro">Registro</label>
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                        title="Adicione os custos para transferência do imóvel.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="registro" name="registro"
                        placeholder="Digite o valor do registro" value="<?= $registro ?>" required>
                    <span class="error-message" id="registro-error"></span>
                </div>
                <div class="form-group">
                    <label for="reforma">Reforma</label>
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                        title="Estime o orçamento necessário para eventuais reformas no imóvel.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="reforma" name="reforma"
                        placeholder="Digite o valor da reforma" value="<?= $reforma ?>" required>
                    <span class="error-message" id="reforma-error"></span>
                </div>
                <div class="form-group">
                    <label for="custos">Outros Custos</label>
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                        title="Use este campo para quaisquer outros custos associados à aquisição que não estejam listados acima.">
                        <i class="fa-solid fa-circle-info tooltip-icon"></i>
                    </span>
                    <input type="text" class="form-control" id="custos" name="custos"
                        placeholder="Digite o valor se possuir outros custos" value="<?= $custos ?>" required>
            ...
            <div>
                ...
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Declare os custos associados à venda do imóvel, preencha apenas o campo em porcentagem ou se preferir os valores absolutos.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
            </div>
            <div class="row g-2">
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="corretorEmReais" name="corretor" step="0.01"
                            placeholder="R$1000,00" value="<?= $corretor ?>" required>
                ...
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="corretorEmPorcentagem" step="0.01"
                            name="corretorporc" placeholder="5%" value="<?= $corretorporc ?>">
                        <label for="corretorEmPorcentagem">Corretor Imobiliario em %</label>
                        <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                            title="Informe a comissão do corretor para a venda do imóvel, 6% é um número usual.">
                            <i class="fa-solid fa-circle-info tooltip-icon"></i>
                        </span>
                    </div>
                </div>
            ...
            <div class="row g-2">
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="asses2EmReais" name="asses2" placeholder="R$1000,00"
                            step="0.01" value="<?= $asses2 ?>" required>
                ...
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="asses2EmPorcentagem" step="0.01" name="asses2porc"
                            placeholder="5%" value="<?= $asses2porc ?>">
                        <label for="asses2EmPorcentagem">Assessoria em %</label>
                        <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                            title="Caso utilize serviços de assessoria, informe o valor a ser desembolsado no momento da venda do imóvel.">
                            <i class="fa-solid fa-circle-info tooltip-icon"></i>
                        </span>
                    </div>
                </div>
            ...
            <div>
                ...
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Este campo calcula automaticamente o IR de acordo com a opção de pessoa Física ou Jurídica. Para situações específicas, use o campo Imposto de Renda Manual.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
            </div>
            <div class="ir-options">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                        value="Imposto de Renda PF" onchange="calcularImposto()" checked>
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault2"
                        value="Imposto de Renda PJ" onchange="calcularImposto()">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="flexRadioDefault" id="flexRadioDefault3"
                        value="Imposto de Renda Manual" onchange="calcularImposto()">
            ...
            <div class="row g-2">
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="IREmReais" name="IREmReais" placeholder="R$1000,00"
                            value="<?= $IREmReais ?>" readonly>
            ...
            <div>
                ...
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Detalhe as receitas que serão obtidas com o imóvel, como aluguel ou locação por temporada.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
            </div>
            <div class="form-group">
                ...
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Defina o tempo estimado para começar a receber a receita do imóvel.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
                <input type="number" class="form-control" id="inicioReceita" name="inicioReceita"
                    placeholder="Digite o prazo de inicio da receita" value="<?= $inicioReceita ?>" required>
                <span class="error-message" id="inicioReceita-error"></span>
            </div>
            <div class="form-group">
                <label for="parcelas">Aluguel Mensal com Despesas</label>
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Informe o valor total do aluguel recebido, inclua o valor de IPTU e condomínio, caso seja pago pelo locatário.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
                <input type="text" class="form-control" id="aluguelLiquido" name="aluguelLiquido"
                    placeholder="Digite o valor do aluguel liquido mensal" value="<?= $aluguelLiquido ?>" required>
                <span class="error-message" id="aluguelLiquido-error"></span>
            </div>
            <div class="form-group">
                <label for="parcelas">Duração do aluguel (meses)</label>
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Estime a duração do contrato de aluguel.">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
                <input type="number" class="form-control" id="duracaoAluguel" name="duracaoAluguel"
                    placeholder="Digite a duracao do aluguel" value="<?= $duracaoAluguel ?>" required>
            ...
            <div class="row g-2">
                <div class="col-md">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="IRRecorrente" name="IRRecorrente"
                            placeholder="R$1000,00" step="0.01" value="<?= $IRRecorrente ?>" readonly>
                        <label for="IRRecorrente">Imposto de Renda Recorrente em R$</label>
                        <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                            title="É o imposto de renda sobre o aluguel. Este campo é calculado conforme a seleção IRPF ou IRPJ. Caso tenha selecionado a opção Manual digite o valor do IR mensal. ">
                            <i class="fa-solid fa-circle-info tooltip-icon"></i>
                        </span>
                    </div>
                </div>
            ...
            <div>
                ...
                <span class="d-inline-block" data-toggle="tooltip" data-html="true"
                    title="Insira o custo mensal do IPTU e Condomínio do imóvel">
                    <i class="fa-solid fa-circle-info tooltip-icon"></i>
                </span>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="IPTU" name="IPTU"
                    placeholder="Digite o prazo de inicio da receita" value="<?= $IPTU ?>" required>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="condominio" name="condominio"
                    placeholder="Digite o valor do aluguel liquido mensal" value="<?= $condominio ?>" required>
            ...
            <div class="modal fade" id="loadDataModal" tabindex="-1" role="dialog" aria-labelledby="loadDataModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" id="carregarDadosButton"
            ...
            <div class="modal fade" id="saveDataModal" tabindex="-1" role="dialog" aria-labelledby="savedDataModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" id="salvarDadosButton"
                                onclick="calculate()">Salvar</button>
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal fade" id="videoModal" tabindex="-1" role="dialog" aria-labelledby="videoModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <iframe width="100%" height="315" src="https://www.youtube.com/embed/XoiOOiuH8iI"
        ...
        if ($requestMethod === "POST") {
            echo
                '<div class="div-choice" style="display: none;">
                <div>
        ...
        <div id="resultado-container">
            <!-- A vista -->
            <div id="title-vista" class="result-title" style="display: none;">Resultado À Vista</div>
            <table id="table-avista" class="table result-table" style="display: none;">
                <tr>
                    <td>Lucro <?= tooltipIcon('Resultado líquido da operação no prazo analisado, considerando receitas e custos do modelo selecionado.') ?>
                    </td>
                    <td><?= $Lucro_Vista ?></td>
                </tr>
                <tr>
                    <td>Capital Necessário <?= tooltipIcon('Maior necessidade de caixa ao longo da operação, considerando o saldo do mês. Ex.: No mês de venda a receita supera os custos acumulados, então, os custos desse mês não são considerados aqui.') ?>
                    </td>
                    <td><?= $Desembolso_Total_Vista ?></td>
                </tr>
                <tr>
                    <td>Taxa de Lucro <?= tooltipIcon('Percentual de lucro da operação em relação ao custo acumulado final.') ?></td>
                    <td><?= $Taxa_Lucro_Vista ?></td>
                </tr>
                <tr>
                    <td>Lance Limite <?= tooltipIcon('Mantendo o valor de venda, indica o maior lance estimado para atingir a rentabilidade desejada.') ?></td>
                    <td><?= $Lance_Rentabilidade_Vista ?></td>
                </tr>
                <tr>
                    <td>TIR (a.m.) <?= tooltipIcon('Taxa Interna de Retorno ao mês, calculada com base no fluxo de caixa da operação.') ?>
                    </td>
                    <td><?= $TIR_AM_Vista ?></td>
                </tr>
                <tr>
                    <td>TIR (a.a.) <?= tooltipIcon('Taxa Interna de Retorno ao ano, derivada da TIR mensal da operação.') ?></td>
                    <td><?= $TIR_AA_Vista ?></td>
                </tr>
                <tr>
                    <td>VPL <?= tooltipIcon('Valor Presente Líquido da operação com base na taxa de desconto informada.') ?>
                    </td>
                    <td><?= $VPL_Vista ?></td>
                </tr>
                <tr>
                    <td>Retorno do Aluguel <?= tooltipIcon('Rentabilidade mensal do aluguel em relação ao custo acumulado da operação, considerando o aluguel líquido (aluguel menos IPTU, condomínio e IR sobre aluguel). Obs.: No gráfico, o aluguel não aparece como redução de custo, e sim como aumento do lucro.') ?>
                    </td>
                    <td><?= $Retorno_Aluguel_Vista ?></td>
                </tr>
            </table>
            ...
            <!-- Price -->
            <div id="title-price" class="result-title" style="display: none;">Resultado Price</div>
            <table id="table-price" class="table result-table" style="display: none;">
                <tr>
                    <td>Lucro <?= tooltipIcon('Resultado líquido da operação no prazo analisado, considerando receitas e custos do modelo selecionado.') ?>
                    </td>
                    <td><?= $Lucro_Price ?></td>
                ...
                <tr>
                    <td>Lance limite <?= tooltipIcon('Mantendo o valor de venda, indica o maior lance estimado para atingir a rentabilidade desejada.') ?></td>
                    <td><?= $Lance_Rentabilidade_Price ?></td>
                </tr>
                <tr>
                    <td>TIR (a.m.) <?= tooltipIcon('Taxa Interna de Retorno ao mês, calculada com base no fluxo de caixa da operação.') ?>
                    </td>
            ...
            <!-- SAC -->
            <div id="title-sac" class="result-title" style="display: none;">Resultado SAC</div>
            <table id="table-sac" class="table result-table">
                <tr>
                    <td>Lucro <?= tooltipIcon('Resultado líquido da operação no prazo analisado, considerando receitas e custos do modelo selecionado.') ?>
                    </td>
                    <td><?= $Lucro_SAC ?></td>
            ...
            <!-- Parcelado -->
            <div id="title-parcelado" class="result-title" style="display: none;">Resultado Parcelado</div>
            <table id="table-parcelado" class="table result-table">
                <tr>
                    <td>Lucro <?= tooltipIcon('Resultado líquido da operação no prazo analisado, considerando receitas e custos do modelo selecionado.') ?>
                    </td>
                ...
                <tr>
                    <td>Lance limite <?= tooltipIcon('Mantendo o valor de venda, indica o maior lance estimado para atingir a rentabilidade desejada.') ?></td>
                    <td><?= $Lance_Rentabilidade_Parcelado ?></td>
                </tr>
                <tr>
                    <td>TIR (a.m.) <?= tooltipIcon('Taxa Interna de Retorno ao mês, calculada com base no fluxo de caixa da operação.') ?>
                    </td>
            ...
            <?php if (!empty($tabelasSensibilidadePorModalidade)): ?>
                <style>
                    #heatmap-container {
                        display: flex;
                    ...
                    .sensibilidade-table-frame {
                        display: inline-block;
                        background: #f1f5f9;
                        padding: 0 4px 4px 0;
                    }

                    .sensibilidade-table-area {
                        display: flex;
                        align-items: center;
                        justify-content: center;
                        gap: 8px;
                    ...
                    .table-label-row .axis-arrow,
                    .table-label-column .axis-arrow {
                        display: inline-block;
                        font-size: 17px;
                        line-height: 1;
                    }
                    ...
                    .sensibilidade-table .axis-corner {
                        width: 44px;
                        max-width: 44px;
                        padding: 0;
                    }

                    .sensibilidade-table thead tr th:first-child {
                        background: #f1f5f9;
                        border-radius: 0;
                    }

                    .sensibilidade-table tbody tr {
                        background: #f1f5f9;
                    }

                    .sensibilidade-table td {
                        width: 68px;
                        height: 56px;
                        padding: 2px;
                        text-align: center;
                    ...
                    .resumo-card.rentabilidade-min strong {
                        color: #dc2626;
                    }

                    .resumo-card.rentabilidade-max strong {
                        color: #16a34a;
                    }

                    .resumo-card.rentabilidade-media strong {
                        color: #1e293b;
                    }

                    .resumo-card.cenarios strong {
                        color: #1e293b;
                    }
                ...
                <div id="heatmap-container">
                    <?php foreach ($tabelasSensibilidadePorModalidade as $modalidadeSensibilidade => $dadosSensibilidade): ?>
                    <section class="sensibilidade-card heatmap-modalidade"
                        data-modalidade="<?= htmlspecialchars($modalidadeSensibilidade) ?>"
                        style="display: none;">
                        <h2>Análise de Sensibilidade - <?= htmlspecialchars($dadosSensibilidade['titulo']) ?></h2>
                        <p>Lucro e rentabilidade em diferentes cenários de venda e arrematação.</p>

                        <div class="table-label-row"><span class="axis-arrow">→</span> Preço de Venda</div>

                        <div class="sensibilidade-table-area">
                            <div class="table-label-column"><span class="axis-arrow">↓</span> Valor de Arrematação</div>
                            <div class="sensibilidade-table-frame">
                                <table class="sensibilidade-table">
                                    <thead>
                                        <tr>
                                            <th class="axis-corner" aria-label="Valor de Arrematação"></th>
                                            <?php foreach ($variacoesSensibilidade as $v): ?>
                                                <th><?= $v > 0 ? '+' : '' ?><?= $v ?>%</th>
                                            <?php endforeach; ?>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php foreach ($variacoesSensibilidade as $i => $varArr): ?>
                                        <tr>
                                            <th><?= $varArr > 0 ? '+' : '' ?><?= $varArr ?>%</th>
                                            <?php foreach ($dadosSensibilidade['tabela'][$i] as $celula): ?>
                                                <td class="<?= classeRoiSensibilidade($celula['roi']) ?>">
                                                    <?php
                                                    // Mostra os principais números do cenário simulado sem poluir a célula.
                                                    $tooltipSensibilidade = sprintf(
                                                        'Arrematação: R$ %s<br>Venda: R$ %s<br>Capital Necessário: R$ %s<br>Lucro: R$ %s',
                                                        number_format($celula['arrematacao'], 2, ',', '.'),
                                                        number_format($celula['venda'], 2, ',', '.'),
                                                        number_format($celula['capitalTotal'] ?? 0, 2, ',', '.'),
                                                        number_format($celula['lucro'], 2, ',', '.')
                                                    );
                                                    ?>
                                                    <div class="cell" data-toggle="tooltip" data-html="true"
                                                        title="<?= htmlspecialchars($tooltipSensibilidade, ENT_QUOTES, 'UTF-8') ?>">
                                                        <div class="roi"><?= number_format($celula['roi'], 1, ',', '.') ?>%</div>
                                                        <?php if (!empty($celula['valor'])): ?>
                                                            <div class="valor"><?= number_format($celula['valor'], 0, ',', '.') ?></div>
                                                        <?php endif; ?>
                                                    </div>
                                                </td>
                                            <?php endforeach; ?>
                        ...
                        <h3 class="resumo-title">Lucro esperado por preço de venda</h3>
                        <div class="resumo">
                            <div class="resumo-card rentabilidade-min">
                                <span>Venda a R$ <?= number_format($dadosSensibilidade['precoVendaMenos10'] ?? 0, 0, ',', '.') ?></span>
                                <strong>Lucro: R$ <?= number_format($dadosSensibilidade['lucroVendaMenos10'], 0, ',', '.') ?></strong>
                            </div>
                            <div class="resumo-card rentabilidade-media">
                                <span>Venda a R$ <?= number_format($dadosSensibilidade['precoVendaBase'] ?? 0, 0, ',', '.') ?></span>
                                <strong>Lucro: R$ <?= number_format($dadosSensibilidade['lucroVendaBase'], 0, ',', '.') ?></strong>
                            </div>
                            <div class="resumo-card rentabilidade-max">
                                <span>Venda a R$ <?= number_format($dadosSensibilidade['precoVendaMais10'] ?? 0, 0, ',', '.') ?></span>
                                <strong>Lucro: R$ <?= number_format($dadosSensibilidade['lucroVendaMais10'], 0, ',', '.') ?></strong>
                            </div>
                        </div>
                    </section>
                    <?php endforeach; ?>
...
<script>
    ...
    var emailCliente = <?php echo json_encode($email); ?>;
        ...
        fetch('search_data.php', {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    document.getElementById('rentabilidade').value = jsonData.rentabilidade || '0%';
                    ...
                    saveRadioState();
                    normalizarCamposTextoDoFormulario();
                    calcularImposto();
    ...
    $(document).ready(function () {
        ...
        function initializeChart(ctx, data) {
            if (!window.Chart || !ctx || !data || !data.labels || !data.datasets) {
                return null;
            }

            var xTickLimit = Math.min(data.labels.length, 18);
        ...
        charts.meuGrafico1 = initializeChart(chart1Canvas ? chart1Canvas.getContext('2d') : null, chartData1);
        charts.meuGrafico2 = initializeChart(chart2Canvas ? chart2Canvas.getContext('2d') : null, chartData2);
        charts.meuGrafico3 = initializeChart(chart3Canvas ? chart3Canvas.getContext('2d') : null, chartData3);
        charts.meuGrafico4 = initializeChart(chart4Canvas ? chart4Canvas.getContext('2d') : null, chartData4);

        window.bidmapCharts = charts;
    });

    <?php if ($requestMethod === "POST"): ?>
        document.addEventListener("DOMContentLoaded", function () {
            var fimDaPaginaElement = document.getElementById('resultado-container');
    ...
    function toggleElements(element, graph, title, isChecked) {
        if (isChecked) {
            ...
            $(element + ', ' + graph).show();
            $(element + ', ' + title).show();
            var chart = window.bidmapCharts && window.bidmapCharts[graph.replace('#', '')];
        ...
        updateResultChoicePosition();
    }

    function updateResultChoicePosition() {
        ...
        var visibleTitle = $('#title-vista:visible, #title-price:visible, #title-sac:visible, #title-parcelado:visible').first();

        if (visibleTitle.length) {
            choice.insertBefore(visibleTitle);
            choice.show();
        } else {
            choice.hide();
        }
    ...
    function updateHeatmapPosition() {
        ...
        var mapas = [
            { grafico: '#meuGrafico3', modalidade: 'vista' },
            { grafico: '#meuGrafico2', modalidade: 'price' },
            { grafico: '#meuGrafico1', modalidade: 'sac' },
            { grafico: '#meuGrafico4', modalidade: 'parcelado' }
        ];

        mapas.forEach(function (item) {
            var grafico = $(item.grafico);
            var heatmap = $('.heatmap-modalidade[data-modalidade="' + item.modalidade + '"]');

            if (!grafico.length || !heatmap.length) {
                return;
            }

            if (grafico.css('display') !== 'none') {
                heatmap.insertBefore(grafico);
                heatmap.show();
            } else {
                heatmap.hide();
            }
    ...
    $(document).ready(function () {
        ...
        setTimeout(updateHeatmapPosition, 150);
    ...
    function formatarPorcentagem(valor) {
        ...
        if (!isFinite(valor)) {
            return '0%'; // Retorna 0 formatado como porcentagem
        }
    ...
    function isCampoPorcentagem(inputElement) {
        return [
            'rentabilidade',
            'porcentagem1',
            'porcentagem2',
            'porcentagem4',
            'corretorEmPorcentagem',
            'asses2EmPorcentagem',
            'asses1EmPorcentagem',
            'ITBIEmPorcentagem',
            'comissaoEmPorcentagem'
        ].includes(inputElement.id);
    }

    function aplicarFormatacaoComBaseNoID(inputElement) {
        if (isCampoPorcentagem(inputElement)) {
            const valor = removerFormatacao(inputElement.value);
    ...
    function normalizarCamposTextoDoFormulario() {
        const camposTexto = document.querySelectorAll('#myForm input[type="text"]');
    ...
    document.addEventListener('focus', function (e) {
        ...
        if (e.target && e.target.type === 'text' && e.target.id !== 'savedCode' && e.target.id !== 'codeTitle') {
            e.target.value = removerFormatacao(e.target.value);
        }
    }, true);

    document.addEventListener('blur', function (e) {
        if (e.target && e.target.type === 'text' && e.target.id !== 'savedCode' && e.target.id !== 'codeTitle') {
            if (isCampoPorcentagem(e.target)) {
    ...
    document.getElementById("limparDadosButton").addEventListener("click", function () {
        ...
        toggleElements('#table-parcelado', '#meuGrafico4', '#title-parcelado', false);
        updateHeatmapPosition();
    ...
    document.getElementById("imprimirButton").addEventListener("click", function () {
        // Reposiciona os heatmaps e espera antes de abrir o print preview para carregar o heatmap.
        updateHeatmapPosition();
    ...
    document.querySelector('form').addEventListener('submit', function (e) {
        ...
        if (isNaN(rentabilidade) || rentabilidade < 0) {
            errorFields1.push('Rentabilidade Desejada');
            updateErrorMessage('rentabilidade', '"Rentabilidade Desejada" nao pode ser menor que 0.');
        } else {
            updateErrorMessage('rentabilidade', '');
        }
    ...
    function calculaPorcentagemComissao(valorArrematacao, inputComissaoEmReais, campoPorcentagem) {
        ...
        if (!isFinite(valorEmPorcentagem)) {
    ...
    function calcularImposto() {
        ...
        if (opcaoSelecionada === "Imposto de Renda PF") {
            ...
            if (!isFinite(IRRecorrente) || IRRecorrente <= 0) {
                IRRecorrente = 0;
            }
