...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    for ($mes = 1; $mes <= $prazo; $mes++) {
        ...
        $Receita_Recorrente_SAC[$mes] = $Receita_Recorrente[$mes];
        $Receita_Recorrente_Parcelado[$mes] = $Receita_Recorrente[$mes];
        $Receita_Recorrente_Price[$mes] = $Receita_Recorrente[$mes];
        $Receita_Recorrente_Vista[$mes] = $Receita_Recorrente[$mes];

        $Custo_Recorrente_SAC[$mes] = $Custo_Recorrente[$mes];
        $Custo_Recorrente_Parcelado[$mes] = $Custo_Recorrente[$mes];
        $Custo_Recorrente_Price[$mes] = $Custo_Recorrente[$mes];
        $Custo_Recorrente_Vista[$mes] = $Custo_Recorrente[$mes];
        ...
        if ($Receita_Recorrente_SAC[$mes] >= - ($Desembolso_SAC[$mes] + $Custo_Recorrente_SAC[$mes])) {
            $Receita_Recorrente_SAC[$mes] += $Desembolso_SAC[$mes] + $Custo_Recorrente_SAC[$mes];
            $Desembolso_SAC[$mes] = 0;
            $Custo_Recorrente_SAC[$mes] = 0;
        } else {
            if ($Receita_Recorrente_SAC[$mes] > - ($Custo_Recorrente_SAC[$mes])) {
                $Desembolso_SAC[$mes] += $Receita_Recorrente_SAC[$mes] + $Custo_Recorrente_SAC[$mes];
                $Receita_Recorrente_SAC[$mes] = 0;
                $Custo_Recorrente_SAC[$mes] = 0;
            } else {
                $Custo_Recorrente_SAC[$mes] += $Receita_Recorrente_SAC[$mes];
                $Receita_Recorrente_SAC[$mes] = 0;
            }
        }
        ...
        if ($Receita_Recorrente_Parcelado[$mes] >= - ($Desembolso_Parcelado[$mes] + $Custo_Recorrente_Parcelado[$mes])) {
            $Receita_Recorrente_Parcelado[$mes] += $Desembolso_Parcelado[$mes] + $Custo_Recorrente_Parcelado[$mes];
            $Desembolso_Parcelado[$mes] = 0;
            $Custo_Recorrente_Parcelado[$mes] = 0;
        } else {
            if ($Receita_Recorrente_Parcelado[$mes] > - ($Custo_Recorrente_Parcelado[$mes])) {
                $Desembolso_Parcelado[$mes] += $Receita_Recorrente_Parcelado[$mes] + $Custo_Recorrente_Parcelado[$mes];
                $Receita_Recorrente_Parcelado[$mes] = 0;
                $Custo_Recorrente_Parcelado[$mes] = 0;
            } else {
                $Custo_Recorrente_Parcelado[$mes] += $Receita_Recorrente_Parcelado[$mes];
                $Receita_Recorrente_Parcelado[$mes] = 0;
            }
        }
        ...
        if ($Receita_Recorrente_Price[$mes] >= - ($Desembolso_Price[$mes] + $Custo_Recorrente_Price[$mes])) {
            $Receita_Recorrente_Price[$mes] += $Desembolso_Price[$mes] + $Custo_Recorrente_Price[$mes];
            $Desembolso_Price[$mes] = 0;
            $Custo_Recorrente_Price[$mes] = 0;
        } else {
            if ($Receita_Recorrente_Price[$mes] > - ($Custo_Recorrente_Price[$mes])) {
                $Desembolso_Price[$mes] += $Receita_Recorrente_Price[$mes] + $Custo_Recorrente_Price[$mes];
                $Receita_Recorrente_Price[$mes] = 0;
                $Custo_Recorrente_Price[$mes] = 0;
            } else {
                $Custo_Recorrente_Price[$mes] += $Receita_Recorrente_Price[$mes];
                $Receita_Recorrente_Price[$mes] = 0;
            }
        }
        ...
        if ($Receita_Recorrente_Vista[$mes] >= - ($Desembolso_Vista[$mes] + $Custo_Recorrente_Vista[$mes])) {
            $Receita_Recorrente_Vista[$mes] += $Desembolso_Vista[$mes] + $Custo_Recorrente_Vista[$mes];
            $Desembolso_Vista[$mes] = 0;
            $Custo_Recorrente_Vista[$mes] = 0;
        } else {
            if ($Receita_Recorrente_Vista[$mes] > - ($Custo_Recorrente_Vista[$mes])) {
                $Desembolso_Vista[$mes] += $Receita_Recorrente_Vista[$mes] + $Custo_Recorrente_Vista[$mes];
                $Receita_Recorrente_Vista[$mes] = 0;
                $Custo_Recorrente_Vista[$mes] = 0;
            } else {
                $Custo_Recorrente_Vista[$mes] += $Receita_Recorrente_Vista[$mes];
                $Receita_Recorrente_Vista[$mes] = 0;
            }
        }
    ...
    if (
        ($receitaRecorrente  - $condominio) > 0 && $arrematacao
        != 0
    ) {
        $Retorno_Aluguel_SAC = ($receitaRecorrente  - $condominio) / -$Desembolso_Total_SAC;
    } else {
    ...
    if (
        ...
        $Retorno_Aluguel_Parcelado = ($receitaRecorrente  - $condominio) / -$Desembolso_Total_Parcelado;
    } else {
    ...
    if (
        ...
        $Retorno_Aluguel_Price = ($receitaRecorrente  - $condominio) / -$Desembolso_Total_Price;
    } else {
    ...
    if (
        ...
        $Retorno_Aluguel_Vista = ($receitaRecorrente  - $condominio) / -$Desembolso_Total_Vista;
    } else {
        ...
        <form method="post" action="#fimDaPagina" id="myForm">
            ...
            <div id="custos-section">
                ...
                <div class="form-group">
                    ...
                    <span class="d-inline-block" data-toggle="tooltip" data-html="true" title="São as dívidas que podem acompanhar o imóvel após a arrematação, normalmente essa informação está no edital">
                        <i class="fa-solid fa-circle-info" style="color: #c7c7c7;"></i>
                    </span>
...
<script>
    ...
    function calcularImposto() {
        ...
        if (opcaoSelecionada === "Imposto de Renda PF") {
            ...
            let custoAquisicao = comissaoEmReais + ITBIEmReais + dividaprop + registro + asses1 + reforma + custos;
